...
# 把Fare這個feature做normalize (x-mean)/std
fare_mean=np.mean(total_data[:,6])  # 計算其平均值
fare_std=np.std(total_data[:,6],ddof=1)  # 計算其標準差
total_data[:,6]=(total_data[:,6]-fare_mean)/fare_std

'''

# 把Age這個feature做normalize (x-mean)/std
age_mean=np.mean(total_data[:,3])  # 計算其平均值
age_std=np.std(total_data[:,3],ddof=1)  # 計算其標準差
total_data[:,3]=(total_data[:,3]-age_mean)/age_std
'''
 
# 把資料分成training與testing      
